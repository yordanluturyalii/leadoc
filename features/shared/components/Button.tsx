...
const Button = ({children, icon, type, isDisable, isDark, handleClick}: ButtonProps) => {
  return (
      <button
          type={type}
          className={`${isDark ? 'bg-neutral-900' : 'bg-white border border-neutral-200 hover:bg-neutral-50'} w-full p-3 flex items-center justify-center gap-2.5 cursor-pointer rounded-xl`}
          onClick={handleClick}>
        {
